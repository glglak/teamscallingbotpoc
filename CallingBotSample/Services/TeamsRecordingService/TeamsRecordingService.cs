...
namespace CallingBotSample.Services.TeamsRecordingService
{
    ...
    public class TeamsRecordingService : ITeamsRecordingService
    {
        ...
        public async Task<byte[]> GetRecordingAsync(string recordingUrl, string accessToken)
        {
            // Use the HttpClient to retrieve the recording from the specified URL
            using (var requestMessage = new HttpRequestMessage(HttpMethod.Get, recordingUrl))
            {
                ...
                {
                    // Return the recording content as a byte array
                    return await response.Content.ReadAsByteArrayAsync();
                }
                else
                {
                    logger.LogError($"Failed to download recording. Status code: {response.StatusCode}");
